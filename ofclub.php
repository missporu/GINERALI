...
mysql_query('UPDATE `ofclub_dopros` SET `start`="0", `kol`="5", `bal`="0" WHERE `id_user` = "'.$user_id.'"');
$summa=_NumFilter($set['lvl']*100+($set['lvl']*100*$set_ofclub_dopros['kol']));
mysql_query('UPDATE `user_set` SET `baks`=`baks`+"'.$summa.'" WHERE `id` = "'.$user_id.'"');
...
if (isset($_POST['go'])) {
    ...
    if (empty($chislo)) {
        ...
        mysql_query("UPDATE `user_set` SET `ofclub_veteran_chislo`='999999999' WHERE `id`='2'");
        $new_chislo=mt_rand(100000001,999999998);
