...
if (empty($_GET['page']) || $_GET['page'] == 0 || $_GET['page'] < 0) {
        ...
        $viso   = _NumRows("SELECT * FROM `mail_dialog` WHERE `id_user`='" . $user_id . "'");
        $puslap = floor($viso / 5);
$data_post=mysql_query("SELECT * FROM `mail_dialog` WHERE `id_user`='" . $user_id . "' OR `id_dialog`='" . $user_id . "' ORDER BY `status` AND `status2` ASC LIMIT $num, 5");
while($vybor_post=mysql_fetch_assoc($data_post)){
if($vybor_post['id_user']==$user_id){
$user_post=_FetchAssoc("SELECT * FROM `user_reg` WHERE `id`='" . $vybor_post['id_dialog'] . "' LIMIT 1");
$set_user_post=_FetchAssoc("SELECT * FROM `user_set` WHERE `id`='" . $user_post['id'] . "' LIMIT 1");
}else{
$user_post=_FetchAssoc("SELECT * FROM `user_reg` WHERE `id`='" . $vybor_post['id_user'] . "' LIMIT 1");
$set_user_post=_FetchAssoc("SELECT * FROM `user_set` WHERE `id`='" . $user_post['id'] . "' LIMIT 1");
}
if($vybor_post['id_dialog']==$user_id AND $vybor_post['status']==0 OR $vybor_post['id_user']==$user_id AND $vybor_post['status2']==0){
...
echo '</div><div class="mini-line"></div><div class="main"><div class="block_zero center">';
        if ($_GET['page'] > 0) {
            echo '<small><b><a href="mail.php?page=' . $back . '"><< Назад </a></small></b>';
        }
        if (empty($_GET['page']) || $_GET['page'] == 0 || $_GET['page'] < $puslap) {
            echo '<small><b><a href="mail.php?page=' . $next . '"> Вперёд >></a></small></b>';
        }
echo'</div></div></div>';
break;
...
$data_id=_NumFilter($_GET['log']);
            ...
            if (strlen($text) < 2 OR strlen($text) > 1500) {
                $_SESSION['err'] = 'Длина сообщения 2-1500 символов.';
