...
switch ($_GET['case']) {
    default:
        ...
        <div class="main"><?
            if ($set['logo'] == 'on') {
                ?><img src="images/logotips/unit.jpg" width="100%" alt="Техника"/>
                <div class="mini-line"></div><?
            }
            ?>
            <div class="menuList">
                <li><a href="unit.php?case=pokupka&tip=1"><img src="images/icons/arrow.png" alt="*"/>Наземная</a></li>
                <li><a href="unit.php?case=pokupka&tip=2"><img src="images/icons/arrow.png" alt="*"/>Морская</a></li>
                <li><a href="unit.php?case=pokupka&tip=3"><img src="images/icons/arrow.png" alt="*"/>Воздушная</a></li>
                <li><a href="unit.php?case=superunit"><img src="images/icons/arrow.png" alt="*"/>Разработки</a></li>
            </div>
            <div class="mini-line"></div>
            <ul class="hint">
                <li>Цена продажи техники уменьшается от цены покупки на 10%.</li>
                <li>Техника используется в битвах с другими игроками, а также при выполнении заданий.</li>
                <li>Возле каждой единицы техники в меню покупки указаны ее название, параметры атаки, защиты, количество
                    уже
                    имеющейся у Вас техники такого вида, а также цена содержания данной единицы техники и ее стоимость.
                </li>
                <li>С достижением новых уровней открывается новая техника, доступная к приобретению.</li>
                <li>Приобретая технику, стоит учитывать не только ее цену, но и цену ее содержания. Каждый час на
                    содержание
                    армии с Вашего счета снимается соответствующая сумма.
                </li>
            </ul>
        </div></div></div><?
        break;
    case 'pokupka':
        ...
        ?>
        <div class="main">
            <div class="block_zero"><span style="color: #999;"><small><?
                        if ($tip == 1) {
            ...
            <div class="block_zero"><span style="color: #999;">Доход:<span
                            style="float: right;"> <?= $set['dohod'] ?></span><br/>Содержание:<span
                            style="float: right;"> <?= $set['soderzhanie'] ?></span><br/>Чистая прибыль:<span
                            style="float: right;"> <?= $set['chistaya'] ?></small></span></span></div><?
            ...
            while ($user_unit = mysql_fetch_assoc($data_unit)) {
                ...
                ?>
                <div class="mini-line"></div>
                <div class="block_zero"><span
                            style="color: #<?= $unit_color ?>;"><?= $user_unit['name'] ?> - <?= $user_unit['kol'] ?></span>
                </div>
                <div class="dot-line"></div>
                <table width="100%">
                <tr>
                    <td width="25%"><img class="float-left" src="images/units/<?= $user_unit['id_unit'] ?>.png"
                                         width="115px" height="80px"
                                         style="margin-left:0px;margin-right:0px;border:1px solid #<?= $unit_color ?>;"
                                         alt="Техника"></td>
                    <td><span style="color: #<?= $unit_color ?>;">Атака/Защита<span
                                    style="float: right;"><?= $user_unit['ataka'] ?>/<?= $user_unit['zaschita'] ?></span><br/>Содержание:<span
                                    style="float: right;"><?= $user_unit['soderzhanie'] ?></span><br/>Цена:<span
                                    style="float: right;"><img src="images/icons/<?= $scr ?>.png"
                                                               alt="*"/><?= $user_unit['cena'] ?></span><br/><form
                                    action="unit.php?case=pokupka&unit=<?= $user_unit['id_unit'] ?>&tip=<?= $user_unit['tip'] ?>&page=<?= $_GET['page'] ?>"
                                    method="POST">Количество:</span><?
                        if ($user_unit['tip'] > 3){
                    ...
                    ?><br/><span class="btn"><span class="end"><input class="label" name="send" type="submit"
                                                                      value="Купить"/></span></span><?
                        if ($user_unit['tip'] < 4){
                        ?><span class="btn"><span class="end"><input class="label" name="prod" type="submit"
                                                                     value="Продать"/></span></span><?
                        }
                        ?></form></td>
                </tr></table><?
            }
            ...
            if ($next_unit) {
                ...
                <div class="block_dashed center"><?= $next_unit['name'] ?><br/><img src="images/units/0.png"
                                                                                    alt="*"/><br/><b><span
                                style="color: #ff3434;">Будет доступно на <?= $next_unit['lvl'] ?> уровне</span></b>
                </div>
                </div><?
            }
    ...
    case 'superunit':
        ...
        while ($user_superunit = mysql_fetch_assoc($data_superunit)) {
            ...
            <table width="100%">
            <tr>
                <td width="45%"><img class="float-left"
                                     src="images/superunits/<?= $user_superunit['id_unit'] . '' . $lock ?>.png"
                                     style="margin-left:5px;margin-right:0px;border:1px solid #999;" alt="Разработка">
                </td>
                <td><b><span style="color: #9c9;"><?= $user_superunit['name'] ?></span></b><br/><span
                            style="color: #999;">Атака<span style="float: right;"><?= $ataka ?></span><br/>Защита<span
                                style="float: right;"><?= $zaschita ?></span><br/>В наличии: <?= $user_superunit['kol'] ?></span>
                </td>
            </tr></table><?
        }
        ...
        <tr>
            <td width="45%"><img class="float-left"
                                 src="images/superunits/<?= $user_sverhsuperunit['id_unit'] . '' . $lock ?>.png"
                                 style="margin-left:5px;margin-right:0px;border:1px solid #999;" alt="Разработка"></td>
            <td><b><span style="color: #9c9;"><?= $user_sverhsuperunit['name'] ?></span></b><br/><span
                        style="color: #999;">Атака<span style="float: right;"><?= $ataka ?></span><br/>Защита<span
                            style="float: right;"><?= $zaschita ?></span><br/>В наличии: <?= $user_sverhsuperunit['kol'] ?></span>
            </td>
        </tr></table><?
        if ($s1['kol'] > $s10['kol'] and $s2['kol'] > $s10['kol'] and $s3['kol'] > $s10['kol'] and $s4['kol'] > $s10['kol'] and $s5['kol'] > $s10['kol'] and $s6['kol'] > $s10['kol'] and $s7['kol'] > $s10['kol'] and $s8['kol'] > $s10['kol'] and $s9['kol'] > $s10['kol']) {
            ?>
            <div class="block_zero center"><a class="btn" href="unit.php?case=sborka"><span class="end"><span
                                class="label">Собрать</span></span></span></a></div><?
        }
...
define('DB_NAME', 'misporu');
define('DB_USER', 'misporu');
define('DB_PASSWORD', 'HGK6BEYNYwrrAZic');
...
class User
{
    ...
    public function register($username, $password)
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO user (login, pass) VALUES (:username, :password)");
        $stmt->bindParam(':username', $username);
...
class Admin extends User
{
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM user WHERE id = :user_id AND role = 'admin'");
